...
{
    fprintf(stderr,
            "Uso: %s --cmd <ping|join|leave> --host <ip> --port <porta>\n",
            program_name);
}
...
int main(int argc, char **argv)
{
    ...
    else if (arguments.command == CLIENT_JOIN)
    {
        ...
        expected_type = M_ACK;
        if (node_config_init(&config, "127.0.0.1", 1U) < 0 ||
            node_init(&node, &config) < 0)
        {
        ...
        if (request.payload == NULL ||
            encode_join_payload(&config,
                                request.payload,
                                JOIN_PAYLOAD_WIRE_SIZE) < 0)
        {
            message_free(&request);
            (void)network_shutdown(socket_fd);
            return EXIT_FAILURE;
    ...
    if (result != PROTOCOL_OK ||
        response.header.message_type != (uint8_t)expected_type ||
        memcmp(response.header.transaction_id,
